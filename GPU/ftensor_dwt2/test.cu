...
//	printtensor(m,n,k,l,w);
  ...
  //  printctensor(m,n,k,l,C);
    ...
    int block=l1;
    float** poo=(float**)A3;
    float (*po)[k1*l1]=(float (*)[k1*l1])poo;
    float* swap3;
    float* swap1;
    float * E1=(float*)malloc(sizeof(float)*m*n*k*l1);
    float * E2=(float*)malloc(sizeof(float)*m*n*k*l1);
    cudaMalloc((void**)&swap3,sizeof(float)*k*l1);
    cudaMalloc((void**)&swap1,sizeof(float)*k1*l1);
    float* swap2=(float*)malloc(sizeof(float)*k*l1);
    float* swap4=(float*)malloc(sizeof(float)*k1*l1);
    printf("haili\n");
    for(int i=0;i<m*n;i++){
    	for(int j=0;j<k1*l1;j++){
    		swap4[j]=po[i][j];
    	}
    cudaMemcpy(swap1,swap4,sizeof(float)*k1*l1,cudaMemcpyHostToDevice);
    dwt_batch<<<block,THREADS>>>(swap1,k1,l1,swap3);
    cudaMemcpy(swap2,swap3,sizeof(float)*k*l1,cudaMemcpyDeviceToHost);
    for(int d=0;d<k*l1;d++){
    			E1[d+i*k*l1]=swap2[d];
    			}
    }
   ...
   /* for(int i=0;i<m*n*k*l1;i++){
    ...
    block=k;
    float** poo1=(float**)E2;
    float (*po1)[k*l1]=(float (*)[k*l1])poo1;
    float* swap5;
    float* swap6;
    cudaMalloc((void**)&swap5,sizeof(float)*k*l1);
    cudaMalloc((void**)&swap6,sizeof(float)*k*l);
    float* swap7=(float*)malloc(sizeof(float)*k*l1);
    float* swap8=(float*)malloc(sizeof(float)*k*l);
    for(int i=0;i<m*n;i++){
    	for(int j=0;j<k*l1;j++){
    		swap7[j]=po1[i][j];
    	}
    	cudaMemcpy(swap5,swap7,sizeof(float)*k*l1,cudaMemcpyHostToDevice);
    	dwt_batch<<<block,THREADS>>>(swap5,l1,k,swap6);
    	cudaMemcpy(swap8,swap6,sizeof(float)*k*l,cudaMemcpyDeviceToHost);
    	for(int d=0;d<k*l;d++){
    		A4[d+i*k*l]=swap8[d];
    	}
    }
...
//    float* d_S1=NULL;
  ...
  /*  for(int i=0;i<(((m<n)?m:n)*k*l+1);i++){
   ...
   // printctensor(m,n,k,l,C);
    ...
    block=k;
   float** poo2=(float**)A4;
   float (*po2)[k*l]=(float (*)[k*l])poo2;
   for(int i=0;i<m*n;i++){
	   for(int j=0;j<k*l;j++){
		   swap8[j]=po2[i][j];
	   }
	   cudaMemcpy(swap6,swap8,sizeof(float)*k*l,cudaMemcpyHostToDevice);
	   idwt_batch<<<k,THREADS>>>(swap6,l2,l1,k,swap5);
	   cudaMemcpy(swap7,swap5,sizeof(float)*k*l1,cudaMemcpyDeviceToHost);
	   for(int d=0;d<k*l1;d++){
	       	E1[d+i*k*l1]=swap7[d];
	       			}
	   }
   ...
   block=l1;
   float** poo3=(float**)E2;
   float (*po3)[k*l1]=(float (*)[k*l1])poo3;
   for(int i=0;i<m*n;i++){
	   for(int j=0;j<k*l1;j++){
		   swap2[j]=po3[i][j];
	   }
	   cudaMemcpy(swap3,swap2,sizeof(float)*k*l1,cudaMemcpyHostToDevice);
	   idwt_batch<<<block,THREADS>>>(swap3,k2,k1,l1,swap1);
	   cudaMemcpy(swap4,swap1,sizeof(float)*k1*l1,cudaMemcpyDeviceToHost);
	   for(int d=0;d<k1*l1;d++){
		   A3[d+i*k1*l1]=swap4[d];
	   }
   }
   /**
    *  int block=l1;
    ...
    float* swap3;
    float* swap1;
    float * E1=(float*)malloc(sizeof(float)*k*l1);
    float * E2=(float*)malloc(sizeof(float)*k*l1);
    cudaMalloc((void**)&swap3,sizeof(float)*k*l1);
    cudaMalloc((void**)&swap1,sizeof(float)*k1*l1);
    float* swap2=(float*)malloc(sizeof(float)*k*l1);
    float* swap4=(float*)malloc(sizeof(float)*k1*l1);
