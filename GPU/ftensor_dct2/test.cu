...
int main(int argc,char* argv[]){
	...
	int num=0;
	float ten;
	cuComplex* tensor=(cuComplex *)malloc(sizeof(cuComplex)*m*n*k*l);
	...
	cufftComplex* G=(cufftComplex*)malloc(m*n*k*l*sizeof(cufftComplex));
	if(!w||!A||!C||!D||!S||!U||!V){
...
//	printtensor(m,n,k,l,w);
	...
	float* B1;
	float* B2;
	float* A3=(float*)malloc(sizeof(float)*m*n*k*l);
	float* A4=(float*)malloc(sizeof(float)*m*n*k*l);
	cudaMalloc((void**)&B1,sizeof(float)*n*m*k*l);
	cudaMalloc((void**)&B2,sizeof(float)*n*m*k*l);
	cudaMalloc((void**)&A1,sizeof(cuComplex)*n*m*k*l);
	cudaMalloc((void**)&A2,sizeof(cuComplex)*n*m*k*l);
	cudaMemcpy(A1,tensor,sizeof(cuComplex)*m*n*k*l,cudaMemcpyHostToDevice);
    if(m*n*k*l<512){
    	threads=m*k*n*l;
    	blocks=1;
    }else{
    ...
    d_tensor_scalar<<<blocks,threads>>>(A2,A1,m,n,k,l);
    cudaMemcpy(G,A2,sizeof(cuComplex)*m*n*k*l,cudaMemcpyDeviceToHost);
	printf("out put G\n");
	for(int i=0;i<m*n*k*l;i++){
			A3[i]=G[i].x;
			}
   ...
   // printctensor(m,n,k,l,C);
    //take  2Dfft
    int block=302*540;
    cudaMemcpy(B1,A3,sizeof(float)*m*n*k*l,cudaMemcpyHostToDevice);
    dct2_batch2<<<block,THREADS>>>(B1,10,3,B2);
    cudaMemcpy(A4,B2,sizeof(float)*m*n*k*l,cudaMemcpyDeviceToHost);
    for(int i=0;i<m*n*k*l;i++){
    			B[i].x=A4[i];
    			B[i].y=0;
    			}
  //  fft_batch(m,n,k,l,G,B);
    printf("output batch dct…………………………………………………………………………\n");
   // printctensor(m,n,k,l,B);
    //transform tensor_scalar to tensor
    //E=tensor_scalartotensor(m,n,k,l,B,E);
    cudaMemcpy(A2,B,sizeof(cuComplex)*m*n*k*l,cudaMemcpyHostToDevice);
    d_tensor_scalar_tensor<<<blocks,threads>>>(A1,A2,m,n,k,l);
   ...
   // cudaMemcpy(A2,E,sizeof(cuComplex)*m*n*k*l,cudaMemcpyHostToDevice);
    d_matview_transpose<<<blocks,threads>>>(A2,A1,m,n,k,l);
    cudaMemcpy(F,A2,sizeof(cuComplex)*m*n*k*l,cudaMemcpyDeviceToHost);
...
//V coo to csr
   ...
   // *** * V
          ...
          cusparseOperation_t transB1=CUSPARSE_OPERATION_NON_TRANSPOSE;
              ...
              cuComplex*  D_data_array=(cuComplex*)malloc(sizeof(cuComplex)*nnzD);
                ...
                d_zero<<<blocks,threads>>>(A1,m*n*k*l);
                if(m*k*l<512){
                                	threads=m*k*l;
                                	blocks=1;
                                }else{
                ...
                d_csrtomatview<<<blocks,threads>>>(A1,d_C_data_array1,d_C_row_array1,d_C_col_array1,m,n,k,l);
                cudaMemcpy(G,A1,sizeof(cuComplex)*m*n*k*l,cudaMemcpyDeviceToHost);
    ...
    //take ifft
    C=tensor_scalar(m,n,k,l,G,C);
    for(int i=0;i<m*n*k*l;i++){
    			A3[i]=C[i].x;
    			}
    cudaMemcpy(B1,A3,sizeof(float)*m*n*k*l,cudaMemcpyHostToDevice);
    idct2_batch2<<<block,THREADS>>>(B1,10,3,B2);
    cudaMemcpy(A3,B2,sizeof(float)*m*n*k*l,cudaMemcpyDeviceToHost);
    for(int i=0;i<m*n*k*l;i++){
       			B[i].x=A3[i];
       			B[i].y=0;
       			}
  //  ifft_batch(m,n,k,l,C,B);
   ...
   /* for(int i=0;i<m*n*k*l;i++){
    ...
    FILE* fp=fopen("/home/haili/Documents/MATLAB/gpu_based_dct2/compress_result9_10.txt","w");
    if(!fp){
