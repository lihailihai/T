 ...
 */
#include "dct.h"
__global__ void dct2_batch(float* A,const int m,const int n,float* B,int batch){
 ...
 * */
__global__ void idct2_batch(float* A,const int m,const int n,float* B,int batch){
	__shared__ float mat[THREADS];
	const int tidx=threadIdx.x;
//	int bidx=blockIdx.x;
	const int t_n=blockDim.x;
