#include"fft.h"
void fft2_batch(int k,int l,cuComplex* t,cuComplex* ft,int batch)
{
	cufftHandle plan;
	int rank=2;
	int N[rank-1]={0};
	N[0]=l;N[1]=k;
		int inembed[rank-1]={0};
		inembed[0]=l;
		inembed[1]=k;
		int onembed[rank-1]={0};
		onembed[0]=l;
		onembed[1]=k;
	int istride=1;
	int ostride=1;
    ...
    stat2=cudaMalloc((void**)&idata,sizeof(cufftComplex)*batch*k*l);
    stat3=cudaMalloc((void**)&odata,sizeof(cufftComplex)*batch*k*l);
    stat4=cudaMemcpy(
    		idata,
    		t,
    		sizeof(cufftComplex)*batch*k*l,
...
void ifft2_batch(int k,int l,cuComplex* t,cuComplex* ft,int batch)
{
